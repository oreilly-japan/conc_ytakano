...
| 4章。P.133。中段の1行目。 | stは先行するメモリ書き込み | stはするメモリ書き込みのみ |
| 4章。P.133。中段の4行目。 | 一方行 | 一方向 |
| 4章。P.139。7行目。 | stは先行するメモリ書き込み | stはメモリ書き込みのみ |
...
まず、P.102で定義する`entering`と、`ticket`ですが、これはそれぞれ、i番目のスレッドがチケットを取得中であるかと、
...
i番目のスレッドがチケットを獲得するには、以下の手順で行います。
...
その後、以下のようにして、自分以外の他のスレッドiの処理が終了するのを待機します。
具体的には以下のように待機します。

1. `entering[i] == false`となるまでループで待機。つまり、他のスレッドがチケットを取得中なら待機。
2. `ticket[i]`の値が自分のチケット番号より小さい、もしくはチケット番号が同じでありかつ自分のスレッド番号の方がiより小さい場合はループで待機。

これを行っているのが、3.9で示した`lock`関数のコードとなります。
